...
import { fetchMothersDayProducts, toVirtualBouquet } from "@/lib/mothersDayProducts";
...
/**
 * /mothers-day + /es/mothers-day — SPA port (pages/MothersDayCollection.tsx).
 *
 * Lives all year round for SEO (SPEC §3: /mothers-day-bouquets-miami 301s
 * here). Outside the purchase window it renders "locked" (coming-soon gate +
 * countdown) but the full content — 7 real products, copy and FAQ — ships in
 * the server HTML so Google indexes a real, non-thin page. ItemList schema
 * uses OutOfStock while locked. The window opens automatically 1 week before
 * Mother's Day and closes 1 week after (computed, no manual flag).
 */

export function mothersDayMetadata(language: Language): Metadata {
...
export default async function MothersDayView({ language }: { language: Language }) {
  const isEs = language === "es";
  const l = (path: string) => localizePath(path, language);
  const raw = await fetchMothersDayProducts();
  const products = raw.map(toVirtualBouquet);
  ...
  const faqs = isEs
    ? [
        ...
        { question: "¿Entregáis flores del Día de la Madre el mismo día en Miami?", answer: "Sí. Amorelia Luxury Floral Gifts entrega flores frescas el mismo día en todo Miami. Pide antes de las 3 PM (hora de Miami) para entrega el mismo día durante la ventana del Día de la Madre." },
    ...
    : [
        ...
        { question: "Do you deliver Mother's Day flowers same-day in Miami?", answer: "Yes. Amorelia Luxury Floral Gifts delivers fresh flowers the same day across Miami. Order before 3 PM (Miami time) for same-day delivery during the Mother's Day window." },
  ...
  const schemaItems = products.map((product) => {
    ...
    return {
      ...
      url: `https://amorelialuxuryfloral.com/bouquets/mothers-day/${product.shopifyHandle}`,
    ...
    <div className="min-h-screen bg-background">
      <JsonLd
        data={breadcrumbSchema([
          { name: isEs ? "Inicio" : "Home", url: base },
          { name: isEs ? "Día de la Madre" : "Mother's Day", url: `${base}/mothers-day` },
        ])}
      />
      <JsonLd
        data={itemListSchema(
      ...
      <div className="pt-24 pb-16">
        <div className="container mx-auto px-6">
          ...
          <div className="text-center mb-10 md:mb-12">
            ...
            <LongTailIntro seoKey="mothers-day" language={language} />
            ...
            <div className="mt-6 flex justify-center">
              <MothersDayCountdown language={language} />
            </div>
          </div>
            ...
            {products.map((product) => {
              ...
              const card = (
                <>
                  ...
                  {/* Card title — NOT a heading: the grid hangs directly off
                      the H1 (an H3 here would skip H1→H3). Same classes/look. */}
                  <p className="font-display text-base md:text-lg font-semibold text-foreground text-center">
                    {product.name}
                  </p>
              ...
              return purchasable ? (
                <Link
                  ...
                  href={l(`/bouquets/mothers-day/${product.shopifyHandle}`)}
          ...
          {/* SEO body — H2 sections + internal link (menor→mayor to Roses). */}
          <div className="max-w-3xl mx-auto mt-14 space-y-6">
            ...
            <div>
              ...
              <p className="font-body text-sm md:text-base text-muted-foreground leading-relaxed">
                {isEs
                  ? "Pide antes de las 3 PM para la entrega el mismo día durante la ventana del Día de la Madre en mayo. Entregamos hasta 90 millas desde nuestra tienda en Miami: $25 de 0 a 5 millas más $1.60 por milla. La recogida en tienda es gratis."
                  : "Order before 3 PM for same-day delivery during the May Mother's Day window. We deliver up to 90 miles from our Miami shop: $25 for 0–5 miles plus $1.60 per mile. Store pickup is free."}
              </p>
            </div>
            <p className="font-body text-sm md:text-base text-muted-foreground leading-relaxed">
      ...
      <LongTailBody seoKey="mothers-day" language={language} />
    </div>
  );
}
