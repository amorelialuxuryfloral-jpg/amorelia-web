 ...
 *    (amorelialuxuryfloral.com / www.amorelialuxuryfloral.com) to keep preview/dev data clean.
 *
 * Pixel ID: PENDIENTE (Amorelia aún no tiene pixel de Meta)
 */
export const META_PIXEL_ID = ""; // Amorelia: pendiente crear pixel de Meta
...
// Events mirrored to the Conversions API (server-side) with the SAME event_id
// so Meta deduplicates browser + server. PageView is included so server-event
// coverage matches the browser pixel (Meta's dedup target ≥75%).
...
const ADD_TO_CART_STORAGE_KEY = "amorelia-meta-addtocart-eventids";
...
const ensureInitialized = (): Fbq | null => {
  ...
  if (!fbq) return null;
  if (!pixelInitialized) {
    if (!META_PIXEL_ID) return null; // Amorelia: pixel pendiente, no inicializar
