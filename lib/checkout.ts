...
export async function performApiCheckout(options: ApiCheckoutOptions): Promise<string | null> {
  ...
  // Charge = 15% of the cart PRODUCTS + EXTRAS total (item.price already
  // includes add-ons; NO delivery/service fee), in $100 brackets — matches
  // what the drawer shows. $15 unit × N brackets.
  const protectedProductsTotal = items.reduce(
    (sum, i) => sum + i.price * (i.quantity || 1),
    0,
  );
  const protectionCharge = computeShippingProtection(
    shippingProtectionInfo ?? getShippingProtectionFallback(),
    protectedProductsTotal,
  );
  ...
  if (options.deliveryMethod === "pickup") {
    ...
    deliveryAddress = {
      address1: "7257 Northwest 12th Street",
      city: "Miami",
      province: "FL",
      zip: "33126",
      country: "US",
