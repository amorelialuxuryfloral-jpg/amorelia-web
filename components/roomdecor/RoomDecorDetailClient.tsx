...
import { roomDecorBouquetColors, type RoomDecorPackage } from "@/lib/roomDecorData";
...
import {
  Check, CalendarIcon, Clock, MapPin, Search, Loader2, Heart, ChevronDown, ChevronUp,
} from "lucide-react";

/**
 * Room Decor configurator — client part of the room decor PDP, ported 1:1
 * from the SPA's RoomDecorDetail. The server page resolves the package,
 * metadata, JSON-LD (Product schema with real-price offers) and the resolved
 * description; everything interactive (options, extras, delivery address via
 * Supabase edge functions, date/time, add-to-cart) lives here.
 *
 * Delivery pricing: room decors get FREE delivery 0–10 miles, $1.60/mile
 * after (calculateRoomDecorDeliveryCost) — no store pickup for this service.
 */

interface Props {
  pkg: RoomDecorPackage;
  /** Resolved description (live Shopify → catalog fallback, done server-side). */
  resolvedDescription: string;
  language?: Language;
}

const RoomDecorDetailClient = ({ pkg, resolvedDescription, language = "en" }: Props) => {
  ...
  // The SPA attached ONE ref to both (dual-rendered) layouts — the mobile one
  // won. Two refs (repo pattern, see ProductDetailClient) keep the exact same
  // markup but make outside-click work on both layouts.
  ...
  // Calendar disabled predicate — SPA rules 1:1 (past days in Miami time +
  // the May 1–12 Mother's Day window).
  const isDateDisabled = (date: Date): boolean =>
    isBefore(startOfDay(date), startOfDay(todayInMiami())) || (date >= new Date(2026, 4, 1) && date <= new Date(2026, 4, 12));
  ...
  const ribbonCost = addRibbon && pkg.ribbonOption && ribbonText.trim() ? pkg.ribbonOption.price : 0;
  ...
  const handleAddToCart = async (): Promise<boolean> => {
    ...
    try {
      ...
      await addItem({
        ...
        // Display split for the cart drawer: base = the package price alone;
        // paid add-ons (banner/ribbon) land in the "Extras" line.
        basePrice: pkg.price,
        deliveryCost,
        totalPrice,
        addons,
    ...
    <>
      ...
      <div className="hidden lg:grid lg:grid-cols-[minmax(0,11fr)_minmax(0,9fr)] gap-8 max-w-6xl mx-auto">
        ...
        <div className="sticky top-24 self-start space-y-3 min-w-0">
          <div className="relative overflow-hidden rounded-lg bg-muted flex items-center justify-center aspect-square">
             <img src={pkg.image} alt={`${pkg.name} Miami Room Decoration – Amorelia Luxury Floral Gifts`} width={600} height={600} className="w-full h-full object-cover" />
          </div>
        </div>
        ...
        <div className="min-w-0 space-y-5 lg:space-y-7">
          <div>
            {/* Rating + trust bar — between the gallery and the title (real
                5.0 / 3 Google business rating, validated claims only — same
                component as the bouquet fichas). */}
            ...
            {/* The full description now lives in ProductInfoAccordion (first
                row, closed by default, always in the served HTML) — same
          ...
          {/* Shipping */}
          <Section title={language === "es" ? "Envío" : "Shipping"} step={step++}>
            <div className="space-y-3 p-4 rounded-lg border border-border bg-card mb-4">
                  <p className="font-body font-semibold text-foreground text-sm">{language === "es" ? "Dirección de entrega" : "Delivery address"}</p>
                  ...
                  <div ref={autocompleteDesktopRef} className="relative">
                    ...
                    <div className="relative">
                      <input type="text" value={addressQuery} onChange={(e) => handleAddressInput(e.target.value)} onFocus={() => predictions.length > 0 && setShowPredictions(true)}
                        placeholder={language === "es" ? "Empieza a escribir la dirección..." : "Start typing the address..."}
            ...
            <div className="mb-4">
              ...
              <PopoverLite
                ...
                trigger={
                  <button type="button" onClick={() => setDesktopCalendarOpen(!desktopCalendarOpen)} className="w-full md:w-auto flex items-center gap-2 px-4 py-3 rounded-lg border border-border bg-card font-body text-sm text-foreground hover:border-primary/30 transition-all">
              ...
              >
                <CalendarLite
                  ...
                  onSelect={(d) => { setDeliveryDate(d); setDeliveryHour(""); setDesktopCalendarOpen(false); }}
            ...
            {deliveryDate && (
              <div>
                ...
                ) : <p className="text-sm text-muted-foreground font-body">{language === "es" ? "No hay horas disponibles. Elige otro día." : "No available hours. Select another day."}</p>}
              </div>
            )}
          </Section>
      ...
      <div className="lg:hidden space-y-5 max-w-lg mx-auto">
        ...
        <div className="text-center">
          {/* Rating + trust bar — between the photo and the title (real
              5.0 / 3 Google business rating, validated claims only — same
              component as the bouquet fichas). */}
          <ProductRatingBar language={language} align="center" />
          {/* Mobile visual title — NOT a heading: the desktop column already
              carries the page's single <h1> with this exact text (SPEC §2.5). */}
          <p className="font-display text-3xl font-semibold text-foreground">{pkg.name} — Room Decor Miami</p>
          {/* Description moved to ProductInfoAccordion (closed by default,
              still in the served HTML) — same structure as the bouquet fichas. */}
        </div>
        ...
        {/* Shipping */}
        <Section title={language === "es" ? "Envío" : "Shipping"} step={step + 1}>
          ...
          <div className="mb-4">
            ...
            <PopoverLite
              ...
              trigger={
                <button type="button" onClick={() => setMobileCalendarOpen(!mobileCalendarOpen)} className="w-full flex items-center gap-2 px-4 py-3 rounded-lg border border-border bg-card font-body text-sm text-foreground hover:border-primary/30 transition-all">
                  <CalendarIcon className="w-4 h-4 text-muted-foreground" />
                  {deliveryDate ? format(deliveryDate, "PPP", { locale: enUS }) : "Select a date"}
                </button>
              }
            >
              <CalendarLite
                selected={deliveryDate}
                onSelect={(d) => { setDeliveryDate(d); setDeliveryHour(""); setMobileCalendarOpen(false); }}
                disabled={isDateDisabled}
              />
            </PopoverLite>
          </div>
      ...
      {/* ===== Shared post-purchase-CTA blocks (mobile AND desktop) =====
          Same structure as the bouquet fichas: Google reviews carousel →
          accordion (description first, CLOSED by default; faqs=[] because
          room decor has NO real FAQs — NADA INVENTADO, so no FAQPage schema
          either) → guarantee badges. The closing GoogleReviewsSummary lives
          in the server page, like on the bouquet pages. */}
...
const Section = ({ title, step, subtitle, children }: { title: string; step: number; subtitle?: string; children: React.ReactNode }) => (
  <div>
    <div className="flex items-center gap-2 mb-3">
      ...
      {/* Configurator step labels ("What's Included", "Shipping", …) are UI
...
export default RoomDecorDetailClient;
