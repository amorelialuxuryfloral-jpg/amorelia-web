...
import DeliveryCalculator, { type DeliveryResult, type StructuredAddress } from "@/components/DeliveryCalculator";
...
interface Props {
  ...
  onCheckout: () => void;
  // FedEx (national shipping > 90 miles). Optional — local delivery / pickup unaffected.
...
/** Delivery-method + date/time + totals block on /checkout — SPA port 1:1. */
const CheckoutSummaryBlock = ({
  ...
  isCheckingOut, onCheckout, fedexBouquetRoses, onFedexAttrs, language = "en",
}: Props) => {
  ...
  const protectionAmount = computeShippingProtection(getShippingProtectionFallback(), itemsSubtotal).amount;
  ...
  const [fedexTrigger, setFedexTrigger] = useState<
    { fullAddress: string; structuredAddress: StructuredAddress | null; miles: number } | null
  >(null);
  ...
  // Same disabled rules as the SPA's react-day-picker matchers:
  // before today (Miami), Sundays, and the Mother's Day window May 1–12 2026.
  const isDateDisabled = (date: Date): boolean => {
    const today = todayInMiami();
    const startOf = (d: Date) => new Date(d.getFullYear(), d.getMonth(), d.getDate());
    if (startOf(date) < startOf(today)) return true;
    if (date.getDay() === 0) return true;
    const from = new Date(2026, 4, 1);
    const to = new Date(2026, 4, 12);
    if (startOf(date) >= startOf(from) && startOf(date) <= startOf(to)) return true;
    return false;
  };
    ...
    <div className="p-6 space-y-5">
      ...
      {deliveryMethod === "pickup" && (
        <p className="font-body text-sm text-muted-foreground">
          {t("checkoutSummary.pickupAt")} <span className="font-semibold text-foreground">7257 NW 12th St, Miami, FL 33126</span>
        </p>
      )}
