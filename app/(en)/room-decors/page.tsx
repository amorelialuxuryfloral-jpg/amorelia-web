...
/**
 * /room-decors — Server Component port of the SPA's RoomDecors listing.
 * Static catalog data (no Shopify fetch); all package cards are real <a>
 * links in the server HTML. ItemList schema carries real prices.
 */
...
export default function RoomDecorsPage() {
  ...
  return (
    <div className="min-h-screen bg-background">
      <JsonLd data={breadcrumbSchema([{ name: "Home", url: "https://amorelialuxuryfloral.com" }, { name: "Romantic Room Decoration", url: "https://amorelialuxuryfloral.com/room-decors" }])} />
      <JsonLd
        data={itemListSchema(
          roomDecorPackages.map((pkg) => ({
            name: `${pkg.name} Miami`,
            url: `https://amorelialuxuryfloral.com/room-decors/${pkg.id}`,
            image: pkg.image,
      ...
      <div className="pt-24 pb-16">
        <div className="container mx-auto px-6">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-muted-foreground font-body text-sm mt-3 max-w-lg mx-auto">
              {t("roomDecors.description")}{' '}
               <Link href="/room-decors/love-bomb" className="text-primary hover:underline">Love Bomb</Link>,{' '}
               <Link href="/room-decors/overly-romantic" className="text-primary hover:underline">Overly Romantic</Link>, {t("roomDecors.orThe")}{' '}
               <Link href="/room-decors/deluxe-love-package" className="text-primary hover:underline">Deluxe Love Package</Link>. {t("roomDecors.deliveryIncluded")}
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-8 max-w-5xl mx-auto">
            {roomDecorPackages.map((pkg) => (
              <div key={pkg.id}>
                <Link href={`/room-decors/${pkg.id}`} className="group block">
        ...
        <div className="max-w-3xl mx-auto mb-10 space-y-6">
          <div>
            <h2 className="font-display text-xl md:text-2xl font-semibold text-foreground mb-2">
              {isEs ? "Decoración romántica para cualquier sorpresa" : "Romantic room decor for any surprise"}
            </h2>
            <p className="font-body text-sm md:text-base text-muted-foreground leading-relaxed">
              {isEs
                ? "Aniversarios, San Valentín, una cita en casa o simplemente porque sí: una decoración romántica con flores convierte una noche normal en un momento inolvidable."
                : "Anniversaries, Valentine's Day, a date night in, or just because — romantic room decor with fresh flowers turns an ordinary evening into a moment she'll remember. Every setup combines room decor flowers, candles and rose petals arranged by our team."}
            </p>
          </div>
          <div>
            <h2 className="font-display text-xl md:text-2xl font-semibold text-foreground mb-2">
          ...
          <p className="font-body text-sm md:text-base text-muted-foreground leading-relaxed">
            ...
            <Link href={isEs ? "/es/bouquets/rosas-rojas" : "/bouquets/red-roses"} className="text-primary hover:underline">
